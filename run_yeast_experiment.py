...
DATA_PATH = DATA_DIR / "yeast.arff"
DEFAULT_TITLE = "Yeast Multi-Label Reduction Experiment (K-fold)"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Yeast 多标记约简实验（支持并行和 K 折交叉验证）。"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
        default=DATA_PATH,
        help="ARFF 数据集路径，默认为项目自带的 yeast 数据。",
    )
    parser.add_argument(
        "--folds",
        type=int,

...
DATA_PATH = DATA_DIR / "birds.arff"
DEFAULT_TITLE = "Birds Multi-Label Reduction Experiment (K-fold)"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Birds 多标记约简实验（支持并行和 K 折交叉验证）。"
    )
    parser.add_argument(
        "--data-path",
        type=Path,
        default=DATA_PATH,
        help="ARFF 数据集路径，默认为项目自带的 birds 数据。",
    )
    parser.add_argument(
        "--folds",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=120.0,
        help="单个折中求解约简的超时时间（秒），小于等于 0 表示不设限制。",
    )
    parser.add_argument(
        "--max-reducts",
        type=int,
        default=1,
        help="单个折中最多返回的约简数量，默认仅求最优一个。",
    )
    parser.add_argument(
        "--prefer-greedy",
        action="store_true",
        help="显式改用贪心近似；默认使用 exact single reduct。",
    )
    parser.add_argument(
        "--disable-greedy",
        action="store_true",
...
def main() -> None:
    ...
    for ratio_index, missing_ratio in enumerate(missing_ratios, start=1):
        ...
        run_experiment(
            ...
            max_reducts=(args.max_reducts if args.max_reducts > 0 else None),
            timeout=(args.timeout if args.timeout and args.timeout > 0 else None),
            prefer_greedy=(args.prefer_greedy and not args.disable_greedy),
        )
